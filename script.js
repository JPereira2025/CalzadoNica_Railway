...
// Contadores para IDs (se reinician al recargar la página, pero se actualizan después)
let idCounters = {
    empleados: 1,
    categorias: 1,
    estilos: 1,
...
// --- NORMALIZACIÓN Y RENDERIZADO DE TABLAS ---
...
function generateCategoriaId(payload) {
    ...
    const consecutivo = String(idCounters.categorias++).padStart(3, '0');
    return `CAT-${nombre}-${consecutivo}`;
}

function generateEstiloId(payload) {
    const nombre = payload.nombre ? payload.nombre.substring(0, 3).toUpperCase() : 'XXX';
    const consecutivo = String(idCounters.estilos++).padStart(3, '0');
...
function generateProductoId(payload) {
    ...
    const consecutivo = String(idCounters.productos++).padStart(3, '0');
    return `PROD-${marca}-${consecutivo}`;
}

function generateCodigoId() {
    const consecutivo = String(idCounters.codigos++).padStart(3, '0');
...
function openProductoModal(id = null) {
    ...
    // Cargar los desplegables cada vez que se abre el modal para asegurar datos frescos
...
function handleProductoSubmit(e) {
    ...
    // Validación para asegurar que se seleccionan categoría y estilo
    ...
    // DEPURACIÓN: Muestra en la consola el payload que se enviará
    ...
    apiCall('productos.php', method, payload).done(resp => {
        ...
        // Si el servidor responde con un error específico, muéstralo.
...
function loadCategoriasForDropdown() {
    ...
    return apiCall('categorias.php').done(data => {
        ...
        categorias.forEach(cat => { 
            // DEPURACIÓN: Muestra en la consola los datos de la categoría
            console.log("Categoría encontrada para dropdown:", cat);
            // Asegúrate de que el ID y el nombre se usen correctamente
...
function loadEstilosForDropdown() {
    ...
    return apiCall('estilos.php').done(data => {
        ...
        $sel.empty().append('<option value="">Seleccionar</option>');
        estilos.forEach(est => { 
            // DEPURACIÓN: Muestra en la consola los datos del estilo
...
function setupCarousel() {
    ...
    function showSlide(index) {
        ...
        $(slides[currentSlide]).removeClass('opacity-0').addClass('opacity-100');
        indicators.removeClass('opacity-70').addClass('opacity-50');
        $(indicators[currentSlide]).removeClass('opacity-50').addClass('opacity-70');
    }
    $('#prevBtn').on('click', () => showSlide(currentSlide - 1));
    $('#nextBtn').on('click', () => showSlide(currentSlide + 1));
    indicators.on('click', function() { showSlide($(this).data('slide')); });
...
function handleExport(e) {
    ...
    if (!table) {
        showNotification('No se encontró la tabla para exportar.', 'error');
        return;
    }
...
function handlePrint(e) {
    ...
    const table = document.getElementById(tableId).closest('table');
    if (!table) {
        showNotification('No se encontró la tabla para imprimir.', 'error');
        return;
    }
